...
pub fn lower_rule_toggle(_checkbox_markdown: &str, _file: &str, id_gen: &mut FreshNodeId) -> RuleGraph {
    ...
    graph.nodes.push(IRNode {
        ...
        label: "rule.toggle entry".into(), source_span: None,
    });

    let terminal_id = id_gen.next();
    graph.nodes.push(IRNode {
        id: terminal_id.clone(), kind: IRNodeKind::Terminal,
        label: "rule.toggle exit".into(), source_span: None,
    });
