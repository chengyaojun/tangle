...
import { lowerRuleToggle, type ToggleIRNode } from "../../src/ir/ruleToggle.js";
...
- [x] \`enable_ui\`: Enable UI`;
    ...
    expect((result.graph.nodes[1] as ToggleIRNode).group).toBe("UI");
    expect((result.graph.nodes[1] as ToggleIRNode).style).toBe("highlight");
  });
