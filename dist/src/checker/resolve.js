...
function extractReceiver(title) {
    const match = title.match(/^(\w+)\s*->/);
    return match?.[1] ?? null;
}
function extractMethodName(heading) {
    if (heading.symbolName)
        return heading.symbolName;
    const afterArrow = heading.title.split("->")[1];
    if (!afterArrow)
        return heading.title;
    return afterArrow.replace(/\(.*\)/, "").trim();
}
function buildCallableSignature(heading) {
    const params = [];
    for (const param of heading.params ?? []) {
        let type;
        if (param.typeName) {
