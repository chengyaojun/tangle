...
export function checkExpression(expr, env) {
    const diags = [];
    switch (expr.kind) {
        case "literal": {
            const map = {
        ...
        case "identifier": {
            if (env.variables[expr.name])
                return [env.variables[expr.name], diags];
            if (env.receiver?.fields[expr.name])
                return [env.receiver.fields[expr.name], diags];
            if (["String", "Int", "Bool"].includes(expr.name)) {
                return [{ kind: "primitive", name: expr.name }, diags];
            }
        ...
        case "this": {
            ...
            const recv = { kind: "struct", name: env.receiver.structName, fields: env.receiver.fields, methods: {} };
            return [recv, diags];
        }
        case "memberAccess": {
        ...
        case "match": {
            ...
            let resultType = { kind: "primitive", name: "Bool" };
            ...
            const patternNames = expr.arms.map((a) => a.pattern.kind === "variantPattern" ? a.pattern.name : "_");
