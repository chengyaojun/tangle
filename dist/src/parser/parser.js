...
// ─── Pratt Parser Core ─────────────────────────────────
function parseExpr(state, minPrec) {
...
function infixPrecedence(token) {
    ...
    if (token.kind === "dot" || token.kind === "lparen")
        return 9;
...
function parseGroupOrArrow(state, lparen) {
    ...
    if (peek(state).kind === "fatArrow") {
        advance(state); // =>
        const params = first.kind === "identifier"
...
// ─── Infix Parsers ─────────────────────────────────────
function parseInfix(state, left, token) {
    // Propagation: expr?
    if (token.kind === "question") {
        advance(state);
...
export function parseCodeBody(tokens) {
    const state = createState(tokens);
    const statements = [];
...
function parseStmt(state) {
    ...
    if (token.kind === "let" || token.kind === "const") {
        ...
        if (peek(state).kind === "eq")
            advance(state); // =
        const value = parseExpr(state, 0);
        const kind = kw.kind === "let" ? "let" : "const";
        return { kind, name: nameToken.value, typeAnnotation, value, span: mergeSpan(kw.span, value.span) };
    }
...
function parseTypeAnnotation(state) {
    const token = peek(state);
    if (token.kind === "identifier") {
        if (["String", "Int", "Bool"].includes(token.value)) {
            advance(state);
            return { kind: "primitiveType", name: token.value, span: token.span };
        }
        advance(state);
        // Check for generic <...>
