...
describe("IR data structures", () => {
    it("defines IRNode variants", () => {
        const kinds = ["action", "compute", "decision", "terminal", "error-terminal"];
        kinds.forEach(k => {
            const node = { kind: k, id: "n", label: k, sourceSpan: { file: "t.md", startLine: 1, startColumn: 1, endLine: 1, endColumn: 1 } };
            expect(node.kind).toBe(k);
        });
    });
    it("defines IREdge with guard", () => {
        const edge = { from: "n1", to: "n2", kind: "condition", guard: "x > 0", sourceSpan: { file: "t.md", startLine: 1, startColumn: 1, endLine: 1, endColumn: 10 } };
        expect(edge.guard).toBe("x > 0");
    });
    it("defines IRErrorEdge", () => {
        const errEdge = { from: "n1", errorVariant: "PayFailed", sourceSpan: { file: "t.md", startLine: 1, startColumn: 1, endLine: 1, endColumn: 10 } };
