import type { RuleGraph, IRNode } from "./graph.js";
import { createGraph, resetNodeCounter } from "./graph.js";

// Parse a Mermaid graph TD string into IR nodes and edges
export function lowerRuleFlow(mermaidSource: string, file: string): RuleGraph {
  resetNodeCounter();
  ...
  for (const line of lines) {
    ...
    if (nodeMatch) {
      ...
      let kind: IRNode["kind"] = "action";
      if (label.startsWith("错误:") || label.startsWith("error:")) {
        kind = "error-terminal";
      }
      const node: IRNode = { kind, id, label, sourceSpan: { file, startLine: 1, startColumn: 1, endLine: 1, endColumn: 1 } };
      nodeMap.set(id, node);
