...
`#` 和 `##` 标题主要负责组织结构，默认不得声明可导出的运行时符号。`###` 和 `####` 标题是符号表的主要来源，可以声明可导出的类型和可调用定义。`#####` 和 `######` 标题默认属于其最近的父级定义，不得被外部模块直接导入。
...
**标题标识符提取算法：** 编译器按以下三级优先级从标题文本提取代码标识符：

1. **规则 1 — 显式括号捕获（最高优先级）：** 标题包含英文半角括号 `(identifier)` 且括号内容符合标识符规范 `[a-zA-Z_][a-zA-Z0-9_]*` 时，无条件提取括号内容作为代码标识符。示例：`#### 映射 (map)` → `map`，`#### マッピング (map)` → `map`。

2. **规则 2 — 纯英文智能简写（次优先级）：** 标题不含括号且整行文本本身符合标识符规范时，直接使用标题文本。示例：`### List` → `List`，`#### length` → `length`。若纯英文标题含空格（如 `#### clear all`），编译器必须报告诊断 `TANGLE_HEADING_MULTI_WORD`，建议使用 `clearAll` 或添加显式括号 `(clear)`。

3. **规则 3 — Unicode 降级（兜底）：** 标题不含括号且包含非 ASCII 字符（中日韩等）时，直接使用整行文本作为标识符。现代宿主语言（TS/JS、Go、Python）均原生支持 Unicode 标识符。示例：`#### 确认支付` → `确认支付`。

**大小写对齐契约（静动大分流）：** 仅对规则 1 和规则 2 提取的纯英文标识符强制执行。Unicode 标识符（规则 3）豁免。

| 标题深度 | 管辖作用域 | 强制规则 | 校验正则 | 合规示例 | 违规示例 |
|---------|-----------|---------|---------|---------|---------|
| `#` 一级 | 包上下文 | PascalCase | `^[A-Z][a-zA-Z0-9]*$` | `# Settlement` | `# settlement` |
| `##` 二级 | 命名空间 | PascalCase | `^[A-Z][a-zA-Z0-9]*$` | `## PaymentGateway` | `## orderDomain` |
| `###` 三级 | 类型声明 | PascalCase | `^[A-Z][a-zA-Z0-9]*$` | `### List` | `### list` |
| `####` 四级 | 可调用定义 | camelCase | `^[a-z][a-zA-Z0-9]*$` | `#### getBalance` | `#### Map` |
| `#####` 五级 | 逻辑语义段 | camelCase | `^[a-z][a-zA-Z0-9]*$` | `##### preCheck` | `##### PreCheck` |
| `######` 六级 | 最小执行单元 | camelCase | `^[a-z][a-zA-Z0-9]*$` | `###### test1` | `###### Test_1` |

违反大小写契约时，编译器必须报告诊断 `TANGLE_INVALID_HEADING_CASE`。同一作用域下两个标题提取出相同标识符时，报告 `TANGLE_DUPLICATE_SYMBOL`。
...
Tangle 消除所有显式 @ 指令，语义通过 Markdown 原生结构表达：

| 语义 | 原生表达 | 示例 |
|------|---------|------|
| 错误定义 | 标题 `Error:` 前缀 | `##### Error: 支付失败 (PayFailed)` |
| 弃用标记 | `~~` 删除线 | `### ~~OldConfig~~` |
| 测试用例 | `Test:` 前缀 | `##### Test: 正常流` |
| 规则决策 | `Rule:` 前缀 | `##### Rule: 审批流` |

**隐式程序入口：** 任何四级标题（`####`）下标识符为 `main` 的函数，编译器隐式将其标记为程序入口点。无需显式指令。`main` 天然满足小驼峰（camelCase）和默认公开规则。一个文件中最多只能有一个 `main` 函数。

文件级元数据应当使用 Markdown front matter 表达，例如 `version`、`title`、`author`。

### 3.6 可见性与访问控制

Tangle 采用默认公开 + 下划线隐式私有的极简访问控制契约。

- 三级/四级标题标识符不以下划线 `_` 开头时，自动视为公开符号。
- 标识符以 `_` 开头时，自动视为模块私有，仅当前 `.md` 文件内可见。
- 深度 5-6 标题始终为内部语义段，不可导出。
...
IR 必须保留符号可见性。以下划线 `_` 开头的符号必须标记为私有。私有符号可以在模块内部被引用，不得被外部模块调用。
...
`tangle run ./main.md` 应当执行指定入口文件的隐式 `main` 函数。若省略文件路径，编译器按以下顺序查找默认入口：`./main.md`、`./index.md`、`./src/main.md`。

`tangle test` 应当收集并运行 `@test` 指令定义的测试。

CLI 参数应当以结构体形式注入隐式 `main` 入口函数。
