...
> **语法精炼勘误（2026-06-25）：** (1) `with { }` → 无关键字大括号更新。(2) `Struct -> method` → 隐式方法绑定。(3) 新增标题大小写对齐契约：深度 1-3 PascalCase，深度 4-6 camelCase。(4) 移除 @export，改为下划线隐式私有。(5) 移除 @entry，改为 main 隐式入口契约。详见设计规格 §3.2。
